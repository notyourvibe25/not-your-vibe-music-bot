...
# Small in-process caches reduce repeated PostgreSQL work on low-resource
# Render instances. Cache is best-effort; correctness never depends on it.
register_cache={}
...
def db():
    ...
    try:
        ...
        # Do not run SELECT 1 on every request. The query itself will detect
        # a broken connection; reconnecting here saves a full DB round-trip.
        yield c;c.commit()
    except Exception:
        if c:
            try:c.rollback()
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS user_history(id BIGSERIAL PRIMARY KEY,user_id BIGINT NOT NULL,mood TEXT NOT NULL,channel_id TEXT NOT NULL,message_id BIGINT NOT NULL,action TEXT NOT NULL DEFAULT 'served',source TEXT NOT NULL DEFAULT 'served',sent_at BIGINT NOT NULL);
    ...
    ALTER TABLE broadcast_comments ADD COLUMN IF NOT EXISTS username TEXT;
    ALTER TABLE broadcast_comments ADD COLUMN IF NOT EXISTS first_name TEXT;
    ALTER TABLE broadcast_comments ADD COLUMN IF NOT EXISTS last_name TEXT;'''
    with db() as c:
...
def register(u):
    ...
    if not isinstance(uid,int):
        return
    now=time.time()
    # Avoid a pair of DB writes for every callback/message. We still refresh
    # user metadata at most once per minute and activity once per day.
    ...
    try:
        with db() as c:
            with cur(c) as x:
                x.execute('''INSERT INTO users(user_id,username,first_name,last_name,first_seen,last_seen,total_requests)
                          ...
                          (uid,u.get('username'),u.get('first_name'),u.get('last_name'),ts,ts))
...
def save_track(mood,ch,msg,title=None):
    ...
    with db() as c:
        with cur(c) as x:
            x.execute('''INSERT INTO tracks(mood,channel_id,message_id,created_at,title)
                         VALUES(%s,%s,%s,%s,%s)
                         ON CONFLICT(channel_id,message_id) DO UPDATE SET
                           mood=EXCLUDED.mood,
                           title=CASE
                             WHEN EXCLUDED.title IS NOT NULL THEN EXCLUDED.title
                             WHEN tracks.title IS NULL OR btrim(tracks.title)='' THEN NULL
                             ELSE tracks.title
                           END
                         RETURNING id''',(mood,str(ch),int(msg),int(time.time()),title))
            return x.fetchone() is not None

def counts(force=False):
    global _counts_cache,_counts_cache_at
    now=time.time()
    with _counts_cache_lock:
        if not force and _counts_cache is not None and now-_counts_cache_at < COUNTS_TTL:
    ...
    with db() as c:
        with cur(c) as x:
            ...
            for row in x.fetchall():
                if row['mood'] in r:
                    r[row['mood']]=int(row['count'])
    with _counts_cache_lock:
        _counts_cache=dict(r);_counts_cache_at=now
    return r
...
def record(uid,mood,ch,msg,source='served'):
    with db() as c:
        with cur(c) as x:
            x.execute("INSERT INTO user_history(user_id,mood,channel_id,message_id,action,source,sent_at) VALUES(%s,%s,%s,%s,'served',%s,%s)",
                      (uid,mood,str(ch),int(msg),source,int(time.time())))

def candidates(mood,limit=120):
    with db() as c:
        with cur(c) as x:
...
def radio_track(uid):
    w=radio_weights(uid); cc=counts(); avail=[m for m,c in cc.items() if c>0]
...
def reserve(uid,ch,source='served'):
    ...
    with db() as c:
        with cur(c) as x:
            x.execute('SELECT pg_advisory_xact_lock(%s)',(uid,))
            x.execute("INSERT INTO user_history(user_id,mood,channel_id,message_id,action,source,sent_at) VALUES(%s,%s,%s,%s,'served',%s,%s)",
                      (uid,ch[0],ch[2],ch[1],source,int(time.time())))
    return ch
...
def save_comment(uid,bid,text,username=None,first_name=None,last_name=None):
    with db() as c:
        with cur(c) as x:
            x.execute('''INSERT INTO broadcast_comments
                (broadcast_id,user_id,username,first_name,last_name,comment,created_at)
                VALUES(%s,%s,%s,%s,%s,%s,%s)''',
                (bid,uid,username,first_name,last_name,text,int(time.time())))
            x.execute('DELETE FROM pending_comments WHERE user_id=%s',(uid,))
...
async def backfill_track_titles_async(rows):
    for row in rows:
        if row.get('title') and not is_bad_title(row.get('title')):
            continue
        try:
            ...
            if not msg:
                continue
            title=clean_title(message_title(msg))
            if not title:
                continue
            with db() as c:
                with cur(c) as x:
...
def ensure_row_titles(rows):
    """Repair old rows whose stored title is missing or was saved as CopyMessage."""
    return backfill_track_titles(rows)

def top_liked_text():
    rows=ensure_row_titles(top_liked_tracks(10))
...
def comments_text(limit=20):
    with db() as c:
        with cur(c) as x:
            x.execute('''SELECT c.id,c.broadcast_id,c.user_id,c.username,c.first_name,c.last_name,c.comment,c.created_at,
                                u.username AS live_username,u.first_name AS live_first_name,u.last_name AS live_last_name
                         FROM broadcast_comments c
                         LEFT JOIN users u ON u.user_id=c.user_id
                         ORDER BY c.id DESC LIMIT %s''',(limit,));rows=x.fetchall()
    if not rows:return '💬 COMMENTS\n\nNo comments yet.'
    lines=['💬 LATEST COMMENTS','━━━━━━━━━━━━━━━━━━','']
    for a in rows:
        username=a.get('username') or a.get('live_username')
        first=a.get('first_name') or a.get('live_first_name') or ''
        last=a.get('last_name') or a.get('live_last_name') or ''
        display=('@'+username) if username else (' '.join(x for x in (first,last) if x) or f"User {a['user_id']}")
        txt=(a['comment'] or '').replace('\n',' ')[:180]
        lines.append(f"Comment #{a['id']} • Broadcast #{a['broadcast_id']}\n👤 {display} (ID: {a['user_id']})\n💬 {txt}\n")
    return '\n'.join(lines)
...
def daily_vibe_mood(uid, day=None):
    day=day or datetime.now(ZoneInfo('Asia/Yangon')).date()
    start,end=_day_bounds(day)
    with db() as c:
        with cur(c) as x:
            x.execute('''SELECT mood,COUNT(*) AS plays
                         FROM user_history
                         WHERE user_id=%s AND source='mood'
                           AND sent_at >= %s AND sent_at < %s
                         GROUP BY mood
                         ORDER BY plays DESC,mood ASC LIMIT 1''',(uid,start,end))
            r=x.fetchone()
...
def daily_vibe_track(uid):
    ...
    return stable_pick(rows,f'daily-vibe:{uid}:{today}:{mood}')
    ...
    with db() as c:
        with cur(c) as x:
            x.execute('''SELECT t.id,t.mood,t.message_id,t.channel_id,
                         ...
                         ORDER BY f.created_at DESC,t.id DESC''',(uid,))
            rows=x.fetchall()
    return ensure_row_titles(rows)
...
def track_of_day_playlist(uid):
    rows=liked_tracks(uid)
    if len(rows)<10:return None,len(rows)
    import hashlib
    tz=ZoneInfo('Asia/Yangon')
    ...
    def ordered_for(day):
        seed=day.isoformat()
        return sorted(rows,key=lambda r:hashlib.md5(
            f'{seed}:{uid}:{r["channel_id"]}:{r["message_id"]}'.encode()
        ).hexdigest())

    current=ordered_for(today)
    if n>10:
        prev=ordered_for(today-timedelta(days=1))
        prev_keys={(str(r['channel_id']),int(r['message_id'])) for r in prev[:10]}
        current_keys={(str(r['channel_id']),int(r['message_id'])) for r in current[:10]}
        if current_keys==prev_keys:
            for offset in range(1,n):
                candidate=current[offset:offset+10] if offset+10<=n else current[offset:]+current[:(offset+10-n)]
                candidate_keys={(str(r['channel_id']),int(r['message_id'])) for r in candidate}
                if candidate_keys!=prev_keys:
                    current=candidate+current[:0]
                    break
    ...
    with db() as c:
        with cur(c) as x:
            x.execute("""SELECT t.id,t.mood,t.channel_id,t.message_id,
                       ...
                       ON f.channel_id=t.channel_id AND f.message_id=t.message_id AND f.feedback='like'
                       GROUP BY t.id,t.mood,t.channel_id,t.message_id,t.title
                       ORDER BY likes DESC,t.created_at DESC LIMIT %s""",(limit,)); rows=x.fetchall()
    return ensure_row_titles(rows)
...
def send_special_music(chat,uid,track,header,source='special'):
    if not track:
...
def send_track_of_day_playlist(chat,uid):
    ...
    if not playlist:
        return send(chat,
            f'🎵 TRACK OF THE DAY\n━━━━━━━━━━━━━━━━━━\n\n'
            f'Raver အနေနဲ့ Like {10} ခု မပေးရသေးသဖြင့် BOT မှ Track of the Day Playlist ကို မပို့ပေးနိုင်သေးပါ။\n\n'
            f'❤️ လက်ရှိ Like: {total}/10\n\n'
            f'သီချင်းတွေကို Like 10 ခု ပြည့်အောင်ပေးပြီး ပြန်လာခဲ့ပါ။ ✨',
            mood_menu())
    send(chat,
         '🎵 TRACK OF THE DAY — YOUR 10 LIKED TRACKS\n'
         '━━━━━━━━━━━━━━━━━━\n\n'
         'ဒီနေ့အတွက် မင်း Like လုပ်ထားတဲ့ Track 10 ပုဒ်ကို ရွေးထားပါတယ်။ ✨')
    for i,r in enumerate(playlist,1):
        ...
        if result.get('ok'):
            ...
            reserve(uid,(mood,msg,ch),'track_of_day')
        else:
            log.warning('Track of Day delivery failed uid=%s channel=%s message=%s',uid,ch,msg)
    send(chat,'━━━━━━━━━━━━━━━━━━\n❤️ All 10 tracks are from your existing Likes.\n🔄 Tomorrow’s playlist will be reshuffled.',mood_menu())
...
def send_music(chat,uid,mood,radio=False):
    ...
    if not reserve(uid,track,'radio' if radio else 'mood'):log.warning('History record failed uid=%s channel=%s message=%s',uid,channel,msg)
...
def callback(c):
    ...
    if data=='profile':
        ...
        return
    if data=='daily_vibe':
        answer(c.get('id'),'🔥 Daily Vibe')
        if not daily_vibe_mood(uid):
            send(chat,'🔥 DAILY VIBE\n━━━━━━━━━━━━━━━━━━\n\nဒီနေ့ Mood Menu ကနေ Mood ရွေးပြီး နားထောင်ထားတဲ့ Track မရှိသေးလို့ Daily Vibe ကို မသတ်မှတ်နိုင်သေးပါ။\n\n🎛 Mood တစ်ခုရွေးပြီး နားထောင်ကြည့်ပါ။',mood_menu())
        else:
            send_special_music(chat,uid,daily_vibe_track(uid),'🔥 YOUR DAILY VIBE','daily_vibe')
        return
    if data=='for_you':
        answer(c.get('id'),'🧠 Personal pick');send_special_music(chat,uid,for_you_track(uid),'🧠 PICKED FOR YOU','for_you');return
    if data=='surprise_me':
        answer(c.get('id'),'🎲 Surprise!');send_special_music(chat,uid,surprise_track(uid),'🎲 SURPRISE ME','surprise_me');return
    if data=='trending':
        answer(c.get('id'));send(chat,trending_text(),mood_menu());return
    if data=='track_of_day':
        answer(c.get('id'),'🎵 Track of the Day');send_track_of_day_playlist(chat,uid);return
...
def message(m):
    ...
    if pending and (m.get('text') or '').strip() and not (m.get('text') or '').strip().startswith('/'):
        ...
        save_comment(uid,pending,text,username,first_name,last_name)
        display=('@'+username) if username else (' '.join(x for x in (first_name,last_name) if x) or f'User {uid}')
...
def cleanup_worker():
    """Run TTL cleanup outside user-facing requests."""
    while True:
        try:
            cleanup_pending()
        except Exception:
            log.exception('background cleanup')
        time.sleep(300)


def startup():
    if not BOT_TOKEN or not DATABASE_URL:log.error('BOT_TOKEN and DATABASE_URL are required');return False
    init_db();webhook_setup();start_telethon()
    threading.Thread(target=cleanup_worker,name='cleanup-worker',daemon=True).start()
    log.info('🟢 BOT READY');return True

...
# VERSION: MOOD-PRIORITY UPDATE — prefer never-sent tracks on Mood selection
import asyncio
import json
import logging
import os
    ...
    try:
        with db_connection() as connection:
            with db_cursor(connection) as cursor:
                ...
                # ====================================================
                # MOOD PRIORITY:
                # When the user explicitly selects a Mood, always prefer
                # tracks that have NEVER been sent to this user before.
                #
                # History is checked across ALL recommendation sources
                # (Mood, Radio, For You, Daily Vibe, Surprise Me, etc.).
                # This means an already-sent track is not preferred merely
                # because it came from another feature.
                # ====================================================
                ...
                # 1) FIRST PRIORITY:
                #    Never-sent tracks in this selected mood.
                cursor.execute(
                    """
                    SELECT *
                    FROM tracks t
                ...
                # 2) If every track in this Mood has already been sent,
                #    prefer the least-recently-used tracks from this Mood
                #    and avoid user-unliked tracks.
                cursor.execute(
                    """
                    SELECT t.*
                    FROM tracks t
                ...
                # 3) If the Mood has fewer tracks than the history window,
                #    reuse an older track only as a last resort.
